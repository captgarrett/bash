...
cat<<EOF
      ...
      |   Author : John G & Mickael R                                      |
      |                                                                  |
      |  Comment : Report Script summarize available server information  |
      |            and highlight values in critical state.               |
      |            Please note this version may contains bugs and errors |
      |            Bug reports and any feedback help us make this        |
      |            script better over time and are always welcome.       |
      +------------------------------------------------------------------+
   ...
   Current available options :
      ...
      * -b | --backupinfo       : Backup details
      * -M | --monitoringinfo   : Monitoring details
      * -u | --userinfo         : Users and their information
      * -e | --ecommerce        : Check server compliance
...
function systeminfo() {
  declare -A SYSTEM
  SYSTEM[INFO]=`cat /etc/issue | head -1`
...
function diskinfo() {
   ...
   declare -A RAID
   RAID[INFO]=`/sbin/lsmod | egrep -o '^3w[-_][9]?[x]{3,4}|ips|^megaraid_sas|^megaraid\s|^raid|^parport' | uniq | head`
   case ${RAID[INFO]} in
        3w* )   RAID[TYPE]='3ware'
                RAID[STATUS]=`/usr/sbin/tw_cli /c0 show allunistatus`;;

        ips )   RAID[TYPE]='IBM SERVERRAID'
                RAID[STATUS]=`ipssend GETCONFIG 1 | egrep 'Status of logical' | grep -oi 'Okay' | tr 'a-z' 'A-Z'`;;

        megaraid_sas )  RAID[TYPE]='LSI SAS'
                        RAID[STATUS]=`/usr/sbin/MegaCli -LdPdInfo -aAll | egrep '^State' | grep -oi 'Optimal' | tr 'a-z' 'A-Z'`;;

        megaraid )      RAID[TYPE]='LSI SCSI'
                        RAID[STATUS]=`/usr/sbin/megarc -ldInfo -a0 -Lall | egrep '^Status:' | grep -oi 'Optimal' | tr 'a-z' 'A-Z'`;;

        raid )  RAID[TYPE]='SW RAID'
                RAID[STATUS]=`cat /proc/mdstat`;;

        *)      RAID[TYPE]='\e[33mNot found\e[0m'
                RAID[STATUS]='UNAVAILABLE';;
   esac

   if [ "${RAID[STATUS]}" = 'OPTIMAL' ] || [ "${RAID[STATUS]}" = 'OKAY' ];then
        RAID[STATUS]='\e[32mOptimal\e[0m'
   elif [ "${RAID[STATUS]}" = 'UNAVAILABLE' ];then
        CHKSTATUS warning
        RAID[STATUS]='\e[33mUnavailable\e[0m'
   else
        CHKSTATUS failure
        RAID[STATUS]='\e[31mFailure\e[0m'
   fi

   TITLE DISKINFO $STATUS
   column -t -s';' $tmpfile
   echo ""
   echo -e "   Raid Type;:;${RAID[TYPE]}\n   Raid Status;:;${RAID[STATUS]}" | column -t -s';'
   rm -f $tmpfile
}
...
function networkinfo() {
   declare -A PUBIP
   PUBIP[API]=`curl -s ipinfo.io | sed -r 's/["{}]//g;s/[a-zA-Z]+://g;s/\s{2}+//g'`
   PUBIP[ADDR]=`echo ${PUBIP[API]} | awk -F',' {'print $1'}`
   PUBIP[PTR]=`echo ${PUBIP[API]} | awk -F',' {'print $2'}`
   PUBIP[CITY]=`echo ${PUBIP[API]} | awk -F',' {'print $3'}`
   PUBIP[REGION]=`echo ${PUBIP[API]} | awk -F',' {'print $4'}`
   PUBIP[COUNTRY]=`echo ${PUBIP[API]} | awk -F',' {'print $5'}`
   PUBIP[ORG]=`echo ${PUBIP[API]} | awk -F',' {'print $8'}`

   if [ "${PUBIP[PTR]}" = ' No Hostname' ];then
        CHKSTATUS warning
        PUBIP[PTR]="\e[33m${PUBIP[PTR]}*\e[0m"
   fi
   ...
   echo -e "   PublicIP (main);:; ${PUBIP[ADDR]}
   PTR record;:;${PUBIP[PTR]}
   GeoIP City;:;${PUBIP[CITY]}
   GeoIP Region;:;${PUBIP[REGION]}
   GeoIP Country;:;${PUBIP[COUNTRY]}
   GeoIP Organisation;:;${PUBIP[ORG]}\v" | column -t -s';'
...
if [ ! $1 = "" ]; then
   for argument in $@; do
        ...
        -b | --backupinfo  )    backupinfo ;;
        -M | --monitorinfo )    monitorinfo ;;
        -u | --userinfo    )    userinfo ;;
        -e | --ecomminfo   )    ecomminfo ;;
